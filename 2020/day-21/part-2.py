...
if __name__ == '__main__':
    ...
    while max([len(x) for x in allergens.values()]) > 1:
        for key in [x for x in allergens if len(allergens[x]) == 1]:
            for other in [x for x in allergens if x != key]:
                allergens[other] -= allergens[key]

    allergens = sorted([(x, list(y)[0]) for x, y in allergens.items()])
    print(",".join([x[1] for x in allergens]))
